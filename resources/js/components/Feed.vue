<template>
    <div class="tweets-block">
        ...
        <div v-for="post in posts" :key="post.id" class="tweet first">
            <p>{{ post.content }}</p>
            <p><a class="time-ago scnd-font-color" href="#18">created {{ post.created_at}}; updated {{ post.updated_at }}</a></p>
...
export default {
    ...
    methods: {
        likePost(postId) {
            axios.post(`./../routes/api.php/post/like/${postId}`)
                .then(response => {
        ...
        createComment(postId) {
            axios.post(`./../routes/api.php/comments/create/${postId}`)
                .then(response => {
